...
// the first real daily refresh runs (which needs ANTHROPIC_API_KEY and
// live web search — see scripts/refresh-brands.ts).
//
...
// (with an API key) to replace every brand with real, cited data.
//
...
function buildPlaceholder(entry: (typeof BRAND_CONFIG)[number]): BrandData {
  ...
  return {
    ...
    marketing_spend: {
      ...
      tier: "4_triangulated",
      derivation:
        "Placeholder — this brand has not yet been through a live data refresh, so no real marketing-spend estimate exists yet. Run `npm run refresh` with ANTHROPIC_API_KEY set (or wait for the next scheduled 09:00 IST run) to replace this with a researched, tiered estimate per the Section 6 methodology.",
      source: null,
    },
